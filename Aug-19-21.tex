...
\begin{algorithmic}[1] \Procedure{\textsc{JerrumFilter}}{Input :  Generating
	...
	\For{a given $g \in S$}
		...
		add $e_g$  to the graph $X_A$}
		\If{a cycle is created in $X_A$}
		...
		\State{Let $\{g_t^{\epsilon_t}\}$ be the edge labels on the
		  cycle $C$ with $g_0$ as label of edge incident to $i_0$, 
	  $\epsilon_t \in \{1,-1\}$}
		  \State{Denote $g_0 g_1^{\epsilon_1}, \ldots
		  g_k^{\epsilon_k}$ as $h$ }
		  \State{Remove $g_0$ and add $h$ to the set $A$. If $h$ is
		  identity, ignore by not adding it to $A$}
		\EndIf
	\EndFor
\EndProcedure
\end{algorithmic}
...
%\State{Now, both the edges in the cycle incident to $i_0$
%			  must correspond to $i_0$. (If they didn't then the
%			  neighbour of $i_0$ wouldn't be the least moved
%		  element by the corresponding group element)}
